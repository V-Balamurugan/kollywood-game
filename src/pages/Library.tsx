    ...
    <div className="max-w-7xl mx-auto px-3.5 sm:px-4 py-4 sm:py-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4 mb-6 sm:mb-8">
        <div>
          <button
            onClick={onBack}
            className="flex items-center gap-1.5 text-xs font-bold text-cinema-muted hover:text-white transition-colors mb-2 group"
          >
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] sm:text-xs text-cinema-muted">
                Complete catalogue of {puzzles.length} Tamil cinema masterworks in the arena database.
              </p>
            </div>
          </div>
        </div>
        ...
        {/* View-Only Badge */}
        <div className="flex items-center gap-2 bg-cinema-surface border border-cinema-border/80 px-3 py-1.5 rounded-2xl text-xs text-cinema-muted self-start md:self-auto font-mono">
      ...
      {/* Search & Filter Bar */}
      <div className="glass-card p-3.5 sm:p-4 rounded-3xl border border-cinema-border/70 mb-6 sm:mb-8 space-y-3 shadow-xl">
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-2.5 sm:gap-3">
          {/* Search Box */}
          <div className="relative flex-1">
            <Search className="w-4 h-4 text-cinema-muted absolute left-3.5 top-3.5" />
            <input
              type="text"
              placeholder="Search by Movie, Hero, Heroine, Director, Music, or Song..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-cinema-dark border border-cinema-border focus:border-brand-500 rounded-2xl pl-10 pr-4 py-2.5 text-xs sm:text-sm text-white placeholder:text-cinema-muted/60 focus:outline-none transition-colors"
            />
          </div>
          ...
          <div className="grid grid-cols-2 md:flex items-center gap-2">
            ...
            <select
              ...
              className="bg-cinema-dark border border-cinema-border rounded-xl px-3 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-brand-500 font-bold"
            >
              <option value="all">All Difficulties</option>
              <option value="easy">⚡ Easy (Blockbusters)</option>
              <option value="medium">🔥 Medium (Hits)</option>
      ...
      {filteredPuzzles.length === 0 ? (
        <div className="text-center py-16 glass-card rounded-3xl border border-cinema-border/60">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-5">
          {filteredPuzzles.map((p) => (
            <div
              key={p.id}
              className="glass-card rounded-3xl p-5 border border-cinema-border/80 hover:border-brand-500/50 transition-all flex flex-col justify-between group shadow-xl hover:shadow-brand-500/10"
            >
              <div>
                {/* Header: Poster + Title + Director */}
                <div className="flex items-start gap-3.5 border-b border-cinema-border/50 pb-3.5 mb-3.5">
                  <img
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap mb-1">
                      <span className={`text-[9px] font-black px-2 py-0.5 rounded-lg uppercase ${p.difficulty === 'easy' ? 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/30' :
                        p.difficulty === 'medium' ? 'bg-amber-500/15 text-amber-300 border border-amber-500/30' :
                          'bg-rose-500/15 text-rose-300 border border-rose-500/30'
                        }`}>
                        {p.difficulty}
                      </span>
                      {p.createdBy ? (
                        <span className="text-[9px] font-bold px-2 py-0.5 rounded bg-brand-500/20 text-brand-300 border border-brand-500/40 truncate max-w-[120px]">
                          🎨 {p.createdBy}
                        </span>
                      ) : (
                        <span className="text-[9px] font-semibold px-2 py-0.5 rounded bg-cinema-surface text-slate-300 border border-cinema-border/50">
                          🎬 Curated
                        </span>
                      )}
                    </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs mb-3.5">
                  {/* Hero */}
                  <div className="p-2.5 rounded-xl bg-cinema-surface border border-cinema-border/60 flex items-center gap-2">
                    <img
                    ...
                    <div className="truncate">
                      <span className="text-[9px] text-amber-400 block uppercase font-black">Hero ({p.hero.firstLetter})</span>
                  ...
                  <div className="p-2.5 rounded-xl bg-cinema-surface border border-cinema-border/60 flex items-center gap-2">
                    ...
                    />
                    <div className="truncate">
                      <span className="text-[9px] text-rose-400 block uppercase font-black">Heroine ({p.heroine.firstLetter})</span>
                      <span className="font-bold text-white truncate block">{p.heroine.name}</span>
                    </div>
                  </div>

                  {/* Song */}
                  <div className="p-2.5 rounded-xl bg-cinema-surface border border-cinema-border/60 flex items-center gap-2 col-span-2">
                    ...
                    <div className="truncate flex-1">
                      <span className="text-[9px] text-purple-400 block uppercase font-black">Song ({p.song.firstLetter})</span>
                ...
                {/* Plot Trivia */}
                {p.trivia && (
                  <p className="text-[11px] text-cinema-muted italic border-t border-cinema-border/40 pt-2.5 line-clamp-2">
                    "{p.trivia}"
                  </p>
                )}
              </div>
